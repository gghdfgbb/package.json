/* Phistar Naming Server - With Strict 35-Second Heartbeat Monitoring & Deletion System */
...
const HEARTBEAT_TIMEOUT = 35 * 1000; // 35 seconds - BOT MUST SEND HEARTBEAT WITHIN THIS TIME
...
function deleteBot(botId, reason = 'manual_deletion') {
    try {
        ...
        botDatabase.heartbeatTimestamps.delete(botId);
...
function findOfflineBotByPrefix(prefix) {
    const now = Date.now();
    const availableBots = Array.from(botDatabase.bots.values())
        .filter(bot => {
            if (bot.prefix !== prefix) return false;
            ...
            // Return TRUE if bot is either OFFLINE or NON-COMPLIANT
            return !isOnline || !isCompliant;
        })
        .sort((a, b) => {
            // Sort by oldest heartbeat first (most likely to be available)
            const aHeartbeat = botDatabase.heartbeatTimestamps.get(a.botId) || 0;
            const bHeartbeat = botDatabase.heartbeatTimestamps.get(b.botId) || 0;
            return aHeartbeat - bHeartbeat;
        });

    return availableBots.length > 0 ? availableBots[0] : null;
}
...
function generateDeletionInterface() {
    ...
    const botRows = allBots.map(bot => {
        ...
        const isCompliant = lastHeartbeat ? (now - lastHeartbeat <= HEARTBEAT_TIMEOUT) : false;
        const status = isOnline ? (isCompliant ? '🟢 ONLINE' : '🟡 LATE') : '🔴 OFFLINE';
        ...
        return `
            <tr>
                ...
                <td>${bot.prefix}</td>
                <td>${status}</td>
                <td>${lastHeartbeat ? new Date(lastHeartbeat).toLocaleString() : 'Never'}</td>
                <td>${timeSinceHeartbeat ? Math.floor(timeSinceHeartbeat/1000) + 's ago' : 'N/A'}</td>
                ...
                <div class="section">
                    ...
                    <div class="checkbox-group">
                        <table>
                            ...
                            <tbody>
                                ${botRows || '<tr><td colspan="8" style="text-align: center;">No bots found</td></tr>'}
                            </tbody>
                        </table>
                    </div>
                </div>
...
// Web interface for bot management - FIXED ROUTE
...
// FIXED get-bot-id endpoint - REUSES offline bots immediately
app.post('/get-bot-id', async (req, res) => {
    try {
        ...
        // STRATEGY 1: Check if current bot exists and can be reused
        if (currentId && botDatabase.bots.has(currentId)) {
            ...
            if (isCompliant) {
                // Bot is compliant - keep using same ID
                botId = currentId;
                status = 'reconnected_compliant';
                console.log(`✅ Bot reconnected (compliant): ${currentId}`);
            } else {
                // Current bot exists but is non-compliant - check if we should reuse
                const isActuallyOnline = botDatabase.onlineBots.has(currentId);
                if (!isActuallyOnline) {
                    // Bot is offline - REUSE it immediately
                    botId = currentId;
                    status = 'reused_offline';
                    console.log(`🔄 Reusing OFFLINE bot: ${currentId}`);
                }
            }
        }

        // STRATEGY 2: Find ANY offline bot with same prefix (IMMEDIATE REUSE)
        if (!botId) {
            const offlineBots = Array.from(botDatabase.bots.values())
                .filter(bot => {
                    ...
                    // Return TRUE if bot is OFFLINE or NON-COMPLIANT
                    return !isOnline || !isCompliant;
                })
                .sort((a, b) => {
                    // Sort by oldest heartbeat first (most likely to be available)
            ...
            if (offlineBots.length > 0) {
                // REUSE the first available offline bot
                const botToReuse = offlineBots[0];
                botId = botToReuse.botId;
                status = 'reused_available';
                console.log(`🔄 Reusing available OFFLINE bot: ${botId} (${offlineBots.length} available)`);
            }
        }

        // STRATEGY 3: Create new bot ONLY if no offline bots available
        if (!botId) {
            ...
            status = 'new';
            console.log(`🎯 NEW bot created: ${botId} (no offline bots available)`);
        }

        // ACTIVATE the bot (whether reused or new)
        const bot = botDatabase.bots.get(botId);
        ...
        // Auto-backup on new bot creation only
        if (status === 'new') {
            setTimeout(() => {
                backupDatabaseToDropbox().catch(console.error);
            }, 5000);
        ...
        res.json({
            ...
            message: `Bot ${status.replace(/_/g, ' ')} - Heartbeat required every 35 seconds`
        });
