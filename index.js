/* Phistar Naming Server - Simplified Always-Online System */
...
// SIMPLIFIED get-bot-id endpoint - BOTS CANNOT REQUEST SPECIFIC IDs
app.post('/get-bot-id', async (req, res) => {
    try {
        const { prefix } = req.body; // REMOVED currentId - bots can't request specific IDs
        ...
        const offlineBots = Array.from(botDatabase.bots.values())
            .filter(bot => 
                bot.prefix === prefix && 
                !botDatabase.onlineBots.has(bot.botId)
            );

        if (offlineBots.length > 0) {
            // Use the first available offline bot
            const offlineBot = offlineBots[0];
            ...
            console.log(`🎯 Giving OFFLINE bot: ${offlineBot.botId} (${offlineBots.length - 1} more offline available for ${prefix})`);
        }
...
app.post('/offline', (req, res) => {
    try {
        ...
        if (botDatabase.onlineBots.has(botId)) {
            botDatabase.onlineBots.delete(botId);
            console.log(`📴 Bot ${botId} explicitly set to OFFLINE - Reason: ${reason}`);
        } else {
