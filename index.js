...
app.get('/', (req, res) => {
    ...
    const heartbeatStatus = Array.from(botDatabase.heartbeatTimestamps.entries()).map(([botId, timestamp]) => {
        const timeSince = now - timestamp;
        const status = timeSince > HEARTBEAT_TIMEOUT ? 'OFFLINE' : 'ONLINE';
        const bot = botDatabase.bots.get(botId);
        return { 
            botId, 
            prefix: bot ? bot.prefix : 'unknown',
    ...
    res.json({
        ...
        domain: SHORT_DOMAIN,
        totalBots: botDatabase.bots.size, // ACCURATE COUNT
        onlineBots: botDatabase.onlineBots.size, // ACCURATE COUNT
...
// Enhanced get-bot-id endpoint with strict heartbeat compliance
app.post('/get-bot-id', async (req, res) => {
    try {
        ...
        if (currentId && botDatabase.bots.has(currentId)) {
            ...
            if (isCompliant && !botDatabase.onlineBots.has(currentId)) {
                // Bot is compliant but not marked online - reactivate
...
app.post('/heartbeat', (req, res) => {
    try {
        ...
        if (status === 'online') {
            ...
            if (!botDatabase.onlineBots.has(botId) && botDatabase.bots.has(botId)) {
                const bot = botDatabase.bots.get(botId);
            ...
            } else if (botDatabase.onlineBots.has(botId)) {
...
async function initializeServer() {
    ...
    // Step 3: Start STRICT heartbeat monitoring
