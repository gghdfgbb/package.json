 ...
 */
function setAllBotsOffline(reason = 'manual_all_offline') {
    try {
        const onlineBotsCount = botDatabase.onlineBots.size;
        
        console.log(`📴 Setting ALL bots to offline. Currently online: ${onlineBotsCount}`);
        
        // Clear all online bots
        botDatabase.onlineBots.clear();
        
        console.log(`✅ All bots set to offline. Previous online count: ${onlineBotsCount}`);
        
        // Save database after change
        saveLocalDatabase();

        return {
            success: true,
            message: `All ${onlineBotsCount} bots set to offline`,
            previousOnlineCount: onlineBotsCount,
        ...
        console.error('❌ Set all bots offline error:', error);
        return {
            success: false,
            error: error.message
        };
...
/**
 * Set specific bots to offline
 */
function setBotsOffline(botIds, reason = 'manual_offline') {
    const results = {
        success: [],
        failed: []
    };
    ...
    botIds.forEach(botId => {
        try {
            if (botDatabase.onlineBots.has(botId)) {
                botDatabase.onlineBots.delete(botId);
                results.success.push(botId);
                console.log(`📴 Bot set to offline: ${botId}`);
            } else {
                results.failed.push({
                    botId: botId,
                    error: 'Bot not online or not found'
                });
            }
        } catch (error) {
            results.failed.push({
        ...
        return `
            <tr>
                ...
                <td>
                    ...
                    <button type="button" onclick="setBotOffline('${bot.botId}')" class="btn-offline-single" ${!isOnline ? 'disabled' : ''}>Set Offline</button>
                </td>
            </tr>
        `;
    }).join('');
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .btn-offline-single { background: #4299e1; padding: 6px 12px; font-size: 12px; }
        .btn-offline-single:hover { background: #3182ce; }
        .btn-offline-single:disabled { background: #a0aec0; cursor: not-allowed; }
        .btn-bulk { background: #e53e3e; padding: 12px 24px; font-size: 16px; }
        .btn-bulk:hover { background: #c53030; }
        .btn-offline-all { background: #4299e1; padding: 12px 24px; font-size: 16px; }
        .btn-offline-all:hover { background: #3182ce; }
        .btn-offline-selected { background: #4299e1; padding: 12px 24px; font-size: 16px; }
        .btn-offline-selected:hover { background: #3182ce; }
...
<body>
        ...
        <div class="tab-container">
            ...
            <div id="active-tab" class="tab-content active">
                <div class="section">
                    ...
                    <div class="actions">
                        ...
                        <button class="btn btn-offline-selected" onclick="setSelectedBotsOffline()">Set Selected Offline</button>
                        <button class="btn btn-offline-all" onclick="setAllBotsOffline()">Set ALL Bots Offline</button>
            ...
            <div id="bulk-tab" class="tab-content">
                <div class="section">
                    ...
                    <div class="form-group">
                        <h3>Set Bots Offline</h3>
                        <p>Set all bots to offline status (does not delete them).</p>
                        <button class="btn btn-offline-all" onclick="setAllBotsOffline()">Set ALL Bots Offline</button>
                    </div>
                    
                    <div class="form-group">
                        <h3>Manual Bot ID Operations</h3>
                        <textarea id="manualBotIds" placeholder="Enter bot IDs to operate on, one per line" style="width: 100%; height: 100px; padding: 10px; border: 1px solid #ccc; border-radius: 4px; margin-bottom: 10px;"></textarea>
                        <div style="display: flex; gap: 10px;">
                            <button class="btn btn-bulk" onclick="deleteManualBots()">Delete Listed Bots</button>
                            <button class="btn btn-offline-selected" onclick="setManualBotsOffline()">Set Listed Bots Offline</button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        async function setBotOffline(botId) {
            if (!confirm(\`Set bot \${botId} to offline?\`)) return;
            ...
            try {
                ...
                if (result.success) {
                    showAlert(\`✅ Bot \${botId} set to offline\`, 'success');
                    setTimeout(() => location.reload(), 1000);
                } else {
                    showAlert(\`❌ Failed to set bot offline: \${result.error}\`, 'error');
                }
            } catch (error) {
                showAlert(\`❌ Error: \${error.message}\`, 'error');
            }
        ...
        async function setSelectedBotsOffline() {
            const selectedBots = Array.from(document.querySelectorAll('input[name="botIds"]:checked'))
                .map(checkbox => checkbox.value);
            
            if (selectedBots.length === 0) {
                alert('Please select at least one bot to set offline.');
                return;
            }
            
            if (!confirm(\`Set \${selectedBots.length} selected bot(s) to offline?\`)) return;
            
            try {
                const response = await fetch('/set-bots-offline', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ 
                        botIds: selectedBots,
                ...
                if (result.success) {
                    showAlert(\`✅ \${result.successCount} bot(s) set to offline\`, 'success');
                    setTimeout(() => location.reload(), 1000);
                } else {
                    showAlert(\`❌ Failed to set bots offline: \${result.error}\`, 'error');
                }
            } catch (error) {
                showAlert(\`❌ Error: \${error.message}\`, 'error');
            }
        ...
        async function setAllBotsOffline() {
            if (!confirm('Are you sure you want to set ALL bots to offline status?')) return;
            
            try {
                const response = await fetch('/set-all-offline', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ 
                        authorization: 'PhistarAdmin2025'
                ...
                if (result.success) {
                    showAlert(\`✅ \${result.previousOnlineCount} bot(s) set to offline\`, 'success');
                    setTimeout(() => location.reload(), 1000);
                } else {
                    showAlert(\`❌ Failed to set all bots offline: \${result.error}\`, 'error');
                }
            } catch (error) {
                showAlert(\`❌ Error: \${error.message}\`, 'error');
            }
        ...
        async function setManualBotsOffline() {
            const manualInput = document.getElementById('manualBotIds').value.trim();
            if (!manualInput) {
                alert('Please enter at least one bot ID');
                return;
            ...
            if (!confirm(\`Set \${botIds.length} bot(s) to offline?\`)) return;
...
// ==================== NEW OFFLINE MANAGEMENT ENDPOINTS ====================

// Set all bots to offline
app.post('/set-all-offline', (req, res) => {
    try {
        ...
        const result = setAllBotsOffline('manual_all_offline');
...
// Set multiple bots to offline
app.post('/set-bots-offline', (req, res) => {
    try {
        ...
        const result = setBotsOffline(botIds, 'bulk_offline');
        
        res.json({
            success: true,
            message: `Set ${result.success.length} bot(s) to offline`,
        ...
        console.error('Set bots offline error:', error);
        res.status(500).json({
            success: false,
            error: error.message
        });
...
app.listen(PORT, '0.0.0.0', async () => {
    ...
    console.log(`📴 Bot Offline System: Enabled - New "Set ALL Bots Offline" button available`);
