/* Phistar Naming Server - Simplified Online/Offline Status System */
...
let botDatabase = {
    bots: new Map(),
    onlineBots: new Map(), // Only bots that are currently online
...
/**
 * Find an available offline bot for a given prefix
 */
function findAvailableOfflineBot(prefix) {
    for (const [botId, bot] of botDatabase.bots.entries()) {
        ...
        // Return bot if it matches prefix AND is offline
...
/**
 * Check if all bots for a prefix are online
 */
function areAllBotsOnlineForPrefix(prefix) {
    const prefixBots = Array.from(botDatabase.bots.values())
        .filter(bot => bot.prefix === prefix);
    
    if (prefixBots.length === 0) return false;
    
    // Check if ALL bots for this prefix are online
    return prefixBots.every(bot => botDatabase.onlineBots.has(bot.botId));
}
...
function markBotOnline(botId) {
    ...
    console.log(`🟢 Bot marked ONLINE: ${botId}`);
    saveLocalDatabase();
    
    return { success: true, message: `Bot ${botId} is now online` };
}

/**
 * Mark a bot as offline
 */
function markBotOffline(botId) {
    if (!botDatabase.bots.has(botId)) {
        return { success: false, error: 'Bot not found' };
    }

    botDatabase.onlineBots.delete(botId);
    console.log(`🔴 Bot marked OFFLINE: ${botId}`);
    saveLocalDatabase();
    
    return { success: true, message: `Bot ${botId} is now offline` };
}
...
function generateDeletionInterface() {
    ...
    const botRows = allBots.map(bot => {
        ...
        const lastSeen = bot.lastSeen ? new Date(bot.lastSeen).toLocaleString() : 'Never';
        ...
        return `
            <tr>
                ...
                <td>
                    ...
                    ${isOnline ? 
                        `<button type="button" onclick="forceOffline('${bot.botId}')" class="btn-force-offline">Force Offline</button>` : 
                        `<button type="button" onclick="forceOnline('${bot.botId}')" class="btn-force-online">Force Online</button>`
                    }
                </td>
            </tr>
        `;
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .btn-force-offline { background: #d69e2e; padding: 6px 12px; font-size: 12px; }
        .btn-force-offline:hover { background: #b7791f; }
        .btn-force-online { background: #38a169; padding: 6px 12px; font-size: 12px; }
        .btn-force-online:hover { background: #2f855a; }
...
<body>
                ...
                <div class="section">
                    ...
                    <div class="checkbox-group">
                        <table>
                            ...
                            <tbody>
                                ${botRows || '<tr><td colspan="7" style="text-align: center;">No bots found</td></tr>'}
                            </tbody>
                        </table>
                    </div>
                </div>
    ...
    <script>
        ...
        async function forceOffline(botId) {
            try {
                ...
                if (result.success) {
                    showAlert(\`🔴 Bot \${botId} forced offline\`, 'success');
                    setTimeout(() => location.reload(), 1000);
                } else {
                    showAlert(\`❌ Failed to force offline: \${result.error}\`, 'error');
                }
            } catch (error) {
                showAlert(\`❌ Error: \${error.message}\`, 'error');
            }
        ...
        async function forceOnline(botId) {
            try {
                ...
                if (result.success) {
                    showAlert(\`🟢 Bot \${botId} forced online\`, 'success');
                    setTimeout(() => location.reload(), 1000);
                } else {
                    showAlert(\`❌ Failed to force online: \${result.error}\`, 'error');
                }
            } catch (error) {
                showAlert(\`❌ Error: \${error.message}\`, 'error');
            }
...
app.get('/', (req, res) => {
    ...
    res.json({
        ...
        system: 'Request-based Online/Offline Status',
...
// SIMPLIFIED get-bot-id endpoint - No heartbeat system
app.post('/get-bot-id', async (req, res) => {
    try {
        ...
        // Check if current bot exists and is online
        if (currentId && botDatabase.bots.has(currentId)) {
            const isOnline = botDatabase.onlineBots.has(currentId);
            
            if (isOnline) {
                // Bot is already online - keep using same ID
                botId = currentId;
                status = 'already_online';
                console.log(`✅ Bot already online: ${currentId}`);
            }
        }

        // Find available OFFLINE bot with same prefix
        if (!botId) {
            const availableBot = findAvailableOfflineBot(prefix);
            
            if (availableBot) {
                // Mark this bot as online
                markBotOnline(availableBot.botId);
                botId = availableBot.botId;
                status = 'reactivated_offline';
                console.log(`🔄 Reactivated OFFLINE bot: ${availableBot.botId}`);
            }
        }

        // Check if all bots for this prefix are online
        if (!botId && areAllBotsOnlineForPrefix(prefix)) {
            return res.status(429).json({
                success: false,
                error: `All bots for prefix '${prefix}' are currently online. No available bots.`,
                prefix: prefix,
                onlineCount: Array.from(botDatabase.bots.values()).filter(b => b.prefix === prefix && botDatabase.onlineBots.has(b.botId)).length
            });
        }

        // Create new bot if no available offline bots AND not all are online
        if (!botId) {
        ...
        res.json({
            ...
            message: `Bot identity ${status} - Call /offline endpoint when done`
        });
...
app.post('/online', (req, res) => {
    try {
        ...
        const result = markBotOnline(botId);
        
        if (result.success) {
            res.json({
                success: true,
                message: `Bot ${botId} is now online`,
                botId: botId,
                status: 'online',
                timestamp: new Date().toISOString()
            });
        ...
        console.error('Online endpoint error:', error);
        res.status(500).json({
            success: false,
            error: error.message
        });
...
// Mark bot as offline endpoint - Bots MUST call this when they're done
app.post('/offline', (req, res) => {
    try {
        ...
        const result = markBotOffline(botId);
        
        if (result.success) {
            res.json({
                success: true,
                message: `Bot ${botId} is now offline and available for reuse`,
                botId: botId,
                status: 'offline',
                timestamp: new Date().toISOString()
            });
...
async function initializeServer() {
    console.log('🚀 Initializing Phistar Naming Server with Request-based Online/Offline system...');
    ...
    console.log(`✅ Server initialized with Request-based Online/Offline system`);
}
...
app.listen(PORT, '0.0.0.0', async () => {
    ...
    console.log(`🔄 System: Request-based Online/Offline Status`);
    ...
    console.log(`🔴 Offline bots: ${botDatabase.bots.size - botDatabase.onlineBots.size}`);
